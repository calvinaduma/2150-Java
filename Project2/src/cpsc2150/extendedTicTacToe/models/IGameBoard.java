...
public interface IGameBoard {
    ...
    default public boolean checkForWinner(BoardPosition lastPos){
        ...
        // if any condtition for a win is true, returns true. Else returns false
    ...
    default boolean checkVerticalWin(BoardPosition lastPos, char player){
        ...
        while(counter <getNumToWin()){
            // if check goes out of bounds, end
            if(r+offset >= getNumColumns()) break;
            BoardPosition one = new BoardPosition(r+offset,c);
            // correct player token found and within bounds and no win condition yet
            if(isPlayerAtPos(one, player) && r+offset < getNumColumns() && counter != getNumToWin()){
                counter++;
                offset++;
            }
            // break if wrong player found in sequence
    ...
    default boolean checkDiagonalWin(BoardPosition lastPos, char player){
        ...
        // checking diagonal moving ot the top right and bottom left
        // diagonal of (+1,+1)
        while(counter<getNumToWin()){
        ...
        if(counter<getNumToWin()){
            // counter and offset starts over at 0 since we are now checking diagonal moving to the top left and bottom right
            counter = 0;
            offset = 0;
