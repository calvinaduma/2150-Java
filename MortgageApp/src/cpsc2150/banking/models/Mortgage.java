...
public class Mortgage extends AbsMortgage {
    ...
    public Mortgage(double costH, double downP, int years, ICustomer customerPerson){
        ...
        if (creditSc < 500) {
            rate += VERYBADRATEADD;
        } else if (creditSc < 600) {
            rate += BADRATEADD;
        } else if (creditSc < 700) {
            rate += NORMALRATEADD;
        } else if (creditSc < 750) {
            rate += GOODRATEADD;
        } else {
            rate += 0;
        }
        ...
        payment = (principal*monthlyInterestRate)/(1-(Math.pow(1+monthlyInterestRate,-(years*12))));
    }
    ...
    public boolean loanApproved(){
        ...
        double debtToIncomeRatio = (((monthlyDebtNoLoan+monthlyDebtLoan)*12*getYears())/(customerIncome*getYears()));
        return !(rate >= VERYBADRATEADD || percentDown < MIN_PERCENT_DOWN*100 || debtToIncomeRatio > .40);
    }
